...
const Appointments: React.FC<AppointmentsProps> = ({ setAppointments, appointment }) => {
  ...
  return (
    <div className='max-h-[300px] rounded-lg shadow-xl flex justify-center items-start bg-[#65AAA1] relative'>
      ...
      <div className='absolute right-0 top-0'>
        <MdDelete 
          onClick={(e: MouseEvent) => setClickedOnDelete(true)} 
      ...
      {clickedOnDelete && (
        <div className='absolute z-[1000] w-[150px] top-1 right-1 rounded-lg bg-white text-black px-[5px] py-[8px]'>
          <button 
            onClick={(e: MouseEvent) => handleDelete(e)} 
            className='text-white bg-red-400 px-[7px] py-[5px] rounded-lg'
          >
            Delete
          </button>
          <button 
          ...
          >
            <MdCancel className='text-black' size={"20px"} />
          </button>
        </div>
      )}
